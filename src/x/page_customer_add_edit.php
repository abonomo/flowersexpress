...
class PageCustomerAddEdit
{
	...
	private function verify_input()
	{
		if($f_mode == 'submit')
		{
			...
			if(strlen($this->f_icode) > Config::$DEFAULT_VARCHAR_LEN) $this->show_output('Error: ID Code is too long.');
			if(strlen($this->f_company_name) > Config::$DEFAULT_VARCHAR_LEN) $this->show_output('Error: Company name entry is too long.');
			if(strlen($this->f_contact_name) > Config::$DEFAULT_VARCHAR_LEN) $this->show_output('Error: Contact name entry is too long.');
			if(strlen($this->f_contact_dept) > Config::$DEFAULT_VARCHAR_LEN) $this->show_output('Error: Contact department entry is too long.');
			if(strlen($this->f_office_phone_number) > Config::$DEFAULT_VARCHAR_LEN) $this->show_output('Error: Office phone number  entry is too long.');
			if(strlen($this->f_cell_phone_number) > Config::$DEFAULT_VARCHAR_LEN) $this->show_output('Error: Cell phone number  entry is too long.');
			if(strlen($this->f_fax_number) > Config::$DEFAULT_VARCHAR_LEN) $this->show_output('Error: Fax number  entry is too long.');
			if(strlen($this->f_address_line_1) > Config::$DEFAULT_VARCHAR_LEN) $this->show_output('Error: Address line 1 entry is too long.');
			if(strlen($this->f_address_line_2) > Config::$DEFAULT_VARCHAR_LEN) $this->show_output('Error: Address line 2 entry is too long.');
			if(strlen($this->f_city) > Config::$DEFAULT_VARCHAR_LEN) $this->show_output('Error: City entry is too long.');
			if(strlen($this->f_province) > Config::$DEFAULT_VARCHAR_LEN) $this->show_output('Error: Province entry is too long.');		
			if(strlen($this->f_country) > Config::$DEFAULT_VARCHAR_LEN) $this->show_output('Error: Country entry is too long.');		
			if(strlen($this->f_notes) > Config::$DEFAULT_TEXT_LEN) $this->show_output('Error: Notes entry is too long.');		
		}
	}
			...
			echo('
						  <tr>
							<td><table width="100%" border="0" cellspacing="0" cellpadding="0">
								<tr>
								  <td width="25%" align="right" valign="middle">&nbsp;</td>
								  <td width="75%" align="left" valign="middle" class="text_error">' . $err_msg . '</td>
								</tr>
							</table></td>
						  </tr>
			');
