...
class PageSupplierAddEdit
{
	//*** CONSTANTS ***
	private static $THIS_PAGE = 'page_template.php';
	private static $NEXT_PAGE = 'page_supplier_menu.php';
	...
	private function get_input()
	{
		...
		else if($this->f_mode == 'edit')
		{
			...
			$supplier_info = DB::get_single_row_fq('
				...
				FROM suppliers WHERE id=\'' . $this->f_id . '\''
			);
			
			$this->f_icode = $supplier_info['icode'];
			$this->f_company_name = $supplier_info['company_name'];
			$this->f_contact_name = $supplier_info['contact_name'];
			$this->f_contact_dept = $supplier_info['contact_dept'];
			$this->f_office_phone_number = $supplier_info['office_phone_number'];
			$this->f_cell_phone_number = $supplier_info['cell_phone_number'];
			$this->f_fax_number = $supplier_info['fax_number'];
			$this->f_address_line_1 = $supplier_info['address_line_1'];
			$this->f_address_line_2 = $supplier_info['address_line_2'];
			$this->f_city = $supplier_info['city'];
			$this->f_province = $supplier_info['province'];
			$this->f_country = $supplier_info['country'];
			$this->f_notes = $supplier_info['notes'];	
		}
	...
	private function process_input()
	{
		if($this->f_action == 'submit')
		{
			...
			IO::navigate_to('page_supplier_view.php?f_id=' . $this->f_id);
		}
	}
	...
	private function show_output($outputarray)
	{
		...
		echo('
               <div align="center">
                  <form name="form1" method="post" action="page_supplier_add_edit.php?f_action=submit&f_mode=' . IO::prepout_url($this->f_mode) . (($this->f_mode == 'edit') ? ('&f_id=' . IO::prepout_url($this->f_id)) : '') . '">
